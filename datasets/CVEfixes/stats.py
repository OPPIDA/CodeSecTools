...
def plot_overview():
    ...
    X, Y1, Y2 = (
        ["File and CWE id", "CWE id only", "False positive"],
        [0, 0, 0],
        [0, 0, 0],
    )
    ...
    bars1 = ax.bar(
        X, Y1, label="All matches (multiple match for a same file)", color="pink"
    )
    bars2 = ax.bar(
        X,
        Y2,
        label="Unique matches (at most one match for a same file)",
        color="purple",
    )
    ...
    ax.set_yscale("log")
    fig.suptitle(
        f"SAST Result Validation Against Actual Dataset (Total defects: {defect_number})"
    )
    return fig
...
def plot_coverity_classification():
    ...
    COLORS_COUNT = [
        {v: 0 for k, v in CoverityParser.COLOR_MAPPING.items()}
    ...
    fig.suptitle(
        f"Coverity benchmark against CVEfixes dataset ({cve_found_number} CVE found over {project_number} project)",
        fontsize=16,
    )
    labels = list(CoverityParser.COLOR_MAPPING.keys())
    handles = [
        plt.Rectangle((0, 0), 1, 1, color=CoverityParser.COLOR_MAPPING[label])
        for label in labels
    ]
