...
def plot_coverity_defects():
    ...
    fig.suptitle(
        f"Coverity benchmark against BenchmarkJava dataset ({unique_correct_number} unique correct analysis over {testcode_number} test codes)",
        fontsize=16,
    )
...
def plot(lang_, sast_, force, show, pgf):
    ...
    if sast == "coverity":
        export_dir = CoverityConstants.BenchmarkJava_RESULT_DIR
        figures.extend(
            [
