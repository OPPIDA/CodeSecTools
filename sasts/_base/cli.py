...
from datasets import DATASETS_ALL
from datasets._base.dataset import FileDataset, GitRepoDataset
from sasts._base.graphics import (
    FileDatasetGraphics,
...
class CLIFactory:
    def __init__(self, cli: click.Group, sast: SAST, help_messages: dict):
        self.cli = cli
        self.sast = sast
    ...
    def add_analyze(self, help: str = "") -> None:
        ...
        @click.option(
            "--lang",
            required=True,
            type=click.Choice(self.sast.supported_languages, case_sensitive=False),
            show_choices=True,
        ...
        def analyze(lang: str, force: bool) -> None:
            result_dir = os.path.join(
                self.sast.result_dir, os.path.basename(WORKING_DIR)
            )
            if os.path.isdir(result_dir):
                click.echo(f"Found existing analysis result at {result_dir}")
                if force:
                    shutil.rmtree(result_dir)
                    self.sast.run_analysis(lang, WORKING_DIR, result_dir)
                else:
    ...
    def add_plot(self, help: str = "") -> None:
        ...
        @click.option(
            "--project",
            type=click.Choice(
                self.sast.list_results(project=True), case_sensitive=False
            ),
            help="Project name",
        )
        @click.option(
            "--dataset",
            type=click.Choice(
                self.sast.list_results(dataset=True), case_sensitive=False
            ),
            help="Dataset name",
        )
        @click.option(
