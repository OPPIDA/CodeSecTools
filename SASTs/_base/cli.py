    ...
    def import_(suffix, force):
        ...
        if os.path.isdir(result_dir):
            ...
            if force:
                ...
                click.echo(
                    "Use --force/-f to overwrite it or --suffix to chnage directory name"
                )
                sys.exit(1)
        else:
            Analyzer.save_results(WORKING_DIR, result_dir)
...
def add_plot(cli, Parser, RESULT_DIR, help=""):
    ...
    @click.option(
        ...
        required=True,
        type=click.Choice(
            Parser.list_results(project=True, limit=10), case_sensitive=False
        ),
        show_choices=True,
    ...
    @click.option(
        "--show", required=False, is_flag=True, default=False, help="Show figures"
    )
    @click.option(
        "--pgf",
        required=False,
