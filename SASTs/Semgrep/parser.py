...
class SemgrepFinding(BaseParser.Defect):
    def __init__(self, defect_data):
        super().__init__(
            ...
            cwe_id = int(re.search(r"CWE-(\d+)", defect_data['extra']['metadata']['cwe'][0]).groups()[0]),
            data = defect_data
        )
...
class SemgrepAnalysisResult(BaseParser.AnalysisResult):
    def __init__(self, result_dir, result_data, analysis_log):
        ...
        self.coverage = float(re.search(r"• Parsed lines: ~([\d\.]+)%", analysis_log).groups()[0]) / 100
...
def plot(project_dir, force, show, pgf, limit=10):
    ...
    ax2.bar(X_checkers, Y_checkers, color=map_colors(X_checkers, checker_category, lang))
    ...
    ax3.bar(X_categories, Y_categories, color=map_colors(X_categories, checker_category, lang))
