...
for sast in SASTS_DIR.glob("*.yaml"):
    with mkdocs_gen_files.open(Path("sasts", "data", sast.name), "r") as data_file:  # ty: ignore[unresolved-attribute]
        sast_data = yaml.safe_load(data_file)

    with mkdocs_gen_files.open(
        Path("sasts", "data", f"{sast.stem}.md"), "w"
    ) as md_file:  # ty: ignore[unresolved-attribute]
        md_file.write(sasts_template.render(sast_data))

for dataset in DATASETS_DIR.glob("*.yaml"):
    with mkdocs_gen_files.open(
        Path("datasets", "data", dataset.name), "r"
    ) as data_file:  # ty: ignore[unresolved-attribute]
        dataset_data = yaml.safe_load(data_file)

    with mkdocs_gen_files.open(
        Path("datasets", "data", f"{dataset.stem}.md"), "w"
    ) as md_file:  # ty: ignore[unresolved-attribute]
